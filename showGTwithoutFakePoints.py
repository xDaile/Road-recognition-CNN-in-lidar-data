...
#    print(gtArray)
    ...
    if(class3>0):
        return 3
    return 5 #comment THIS LINE IF WANT TO HAVE 0123 in tensors, otherwise, there will be just 012
    if(class2>0):
        return 2
    return 3
...
def createTensorAndGTFromFile(nameOfPCL):
    ...
    while i < 400:
        ...
        j = 0
        i = i + 1
    emptyGT=[[0 for i in range(200)] for j in range(400)]
    i=0
    while(i<400):
        j=0
        while(j<200):
            if(groundTruth[i][j]==9):
                ...
                emptyGT[i][j]=groundTruth[i][j]
            j=j+1
        i=i+1


#    exit(1)
    nump=np.array(emptyGT)
...
def main():
    ...
    rotatedFiles="umm_010076.poinCL"

    createTensorAndGTFromFile(rotatedFiles)
